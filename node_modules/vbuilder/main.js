 ...
 */
var Cfg, CssCtl, FontsCtl, HtmlCtl, ImgCtl, JsCtl, Lib, Server, SpCtl, TplCtl, Utils, Watcher, fs, gutil, main, opts, path, vfs;
...
main = {
  ...
  init: function() {
    var Paths, key, val, _dir, _makePath, _root;
    gutil.log('初始化项目的目录');
    _root = Cfg.root;
    Paths = opts.paths;
  ...
  clean: function() {
    return gutil.log('构建系统的清理入口');
  },
  cache: function() {
    return gutil.log('初始化Cache入口');
  },
  sprite: function(cb) {
    return new SpCtl(opts).init(function(spmap) {
      return cb && cb();
    });
  },
  img: function(cb) {
  ...
  fonts: function(cb) {
    return new FontsCtl(opts).copyDist(function() {
      return cb && cb();
    });
  },
  watch: function() {
            ...
            return _this.css(function() {
              return _this.js(function() {
                return _this.html(function() {
                  if (opts.isDebug && opts.env !== 'local' && opts.env !== 'dev') {
                    Utils.jsonToPhpArray();
                  }
                  if (opts.env !== 'local' && opts.env !== 'dev') {
                    Utils.jsonToviewPath();
                  }
                  return cb && cb();
                });
              });
