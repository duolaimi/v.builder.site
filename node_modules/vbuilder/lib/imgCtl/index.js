...
ImgCtl = (function() {
  ...
  ImgCtl.prototype.setDistPath = function() {
    var _imgDistPath, _this;
    _this = this;
  ...
  ImgCtl.prototype.copyImgs = function(cb) {
    ...
    return vfs.src([_imgSrcPath + '/*.{gif,jpg,png,svg}', _imgSrcPath + '/**/*.{gif,jpg,png,svg}']).pipe(imagemin({
      quality: '65-80',
      speed: 4
    })()).pipe(vfs.dest(_imgDebugPath)).pipe(vfs.dest(_imgDistPath)).pipe(_this.setDistPath()).pipe(vfs.dest(_imgDistPath)).on('end', function() {
      return cb && cb();
    });
  };
