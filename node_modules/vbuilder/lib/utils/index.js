 ...
 * @version $Id$
 */
var Cfg, Tools, color, crypto, fs, gutil, http, https, opts, path, uglify, _, _url,
...
Tools.getMap = function(type) {
  var e, _ctl, _file, _map, _name, _source, _this;
  _this = Tools;
  _map = {};
...
Tools.buildReqPaths = function(cb) {
  var err, filePath, _cb, _libs, _reqPathsFile;
  _cb = cb || function() {};
...
Tools.buildReqCfg = function(cb) {
  var curHash, e, file, jsLibPaths, key, newPaths, reqCfg, reqCfgStr, reqShim, val, _reqShimFile;
  Tools.getMap("jsHash");
  if (!_.has(global.Cache, 'jsLibs')) {
    Tools.buildReqPaths();
  }
  ...
  reqCfg = {
    baseUrl: opts.cdnDomain + path.join(opts.debugPath, 'js'),
    paths: newPaths,
    shim: reqShim
  };
...
Tools.getGloabVars = function() {
  ...
  GLOBAL_VARS = "var STATIC_PATH='" + _staticPath + "',sbLib = window['sbLib'] = {},_VM_ = window['_VM_'] = {};sbLib.getStaticUri = {img: function(n){return STATIC_PATH+'img/'+n;},css: function(n) {return STATIC_PATH+'css/'+n;},js: function(n) {return STATIC_PATH+'js/'+n}};";
...
Tools.init_js = function(jsList) {
  ...
  if (__indexOf.call(opts.coreJs.mods, "Zepto") >= 0 || __indexOf.call(opts.coreJs.mods, "zepto") >= 0) {
...
Tools.jsonToPhpArray = function(cb) {
  var _cb, _mapPath, _maps, _outPath, _temp;
  _cb = cb || function() {};
  ...
  _maps.forEach(function(key) {
    var file, name, phpFile, val, _arrStr, _hash, _jsonData, _name, _tempArr;
    file = _mapPath + key + '.json';
    phpFile = path.join(_outPath, 'map', key + ".php");
    _jsonData = JSON.parse(fs.readFileSync(file, 'utf8'));
    _tempArr = [];
    ...
    _temp[key] = _tempArr.join();
    _arrStr = '<?php' + '\r\n' + 'return array(' + _tempArr + ');' + '\r\n' + '?>';
    return Tools.writeFile(phpFile, _arrStr);
  });
  return _cb();
};

Tools.jsonToviewPath = function(cb) {
  var _cb, _mapPath, _maps, _outPath, _temp;
  _cb = cb || function() {};
  _mapPath = opts.mapPath;
  _outPath = opts.viewPath;
...
Tools.setDistPath = function(parse, hash) {
  parse.base = parse.name + "." + hash.substring(0, Cfg.hashLen) + parse.ext;
